...
class AdminPostAction extends AdminbaseAction {
	...
	function add_post(){
		...
		if (IS_POST) {
			...
			$_POST['game']['type']=$_POST['term']['term_id'];
	...
	public function edit(){
		...
		$term_id = intval(I("get.term"));
		if(empty($term_id)){
			$term_id = M('GtermRelationships')->where("object_id=$id")->getField('term_id'); 
		}
		$term=$this->terms_obj->where("term_id=$term_id")->find();
		$game=$this->games_obj->where("id=$id")->find();
		$this->assign("game",$game);
		$this->assign("smeta",json_decode($game['smeta'],true));
		$this->assign("term",$term);
		$this->display();
	}
	...
	public function edit_post(){
		if (IS_POST) {
			...
			//echo($this->games_obj->getLastSql());die;
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private function _getTree(){
		...
		foreach ($result as $r) {
			...
			$array[] = $r;
			$terms[$r['term_id']]=$r['name'];
		}
		
		$tree->init($array);
		$str="<option value='\$id' \$selected>\$spacer\$name</option>";
		$taxonomys = $tree->get_tree(0, $str);
		$this->assign("terms", $terms);//分类列表
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			$ids=join(",", $ids);
			if ( $this->games_obj->where("id in ($ids)")->save($data)!==false) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
	...
	function clean(){
		if(isset($_POST['ids'])){
			...
			if($status!==false){
				$status=$this->games_obj->where("id in ($ids)")->delete();
			}
			...
			if(isset($_GET['id'])){
				...
				if($status!==false){
					$status=$this->games_obj->where("id=$id")->delete();
				}
				if ($status!==false) {
					$this->success("删除成功！");
				} else {
